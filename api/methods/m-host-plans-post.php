...
$app->post($route, function ($host) use ($app){
	...
	if(isset($param['name'])){ $name = mysql_real_escape_string($param['name']); } else { $name = 'No Plan Name'; }
	...
	if($hostresult && mysql_num_rows($hostresult))
		...
		if($entryresults && mysql_num_rows($entryresults))
			...
			{
			$query = "INSERT INTO host_plans(host_id,name) VALUES(" . mysql_real_escape_string($host_id) . ",'" . mysql_real_escape_string($name) . "')";
			//echo $query . "<br />";
