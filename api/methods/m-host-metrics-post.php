...
$app->post($route, function ($host) use ($app){
	...
	if($hostresult && mysql_num_rows($hostresult))
		...
		// Add Elements for Host
  		$HostElementQuery = "SELECT * FROM host_metrics WHERE host_id = " . $host_id . " AND name = '" . $name . "'";
		...
		if($HostElementResults && mysql_num_rows($HostElementResults))
			...
			{
			$query = "INSERT INTO host_metrics(host_id,name) VALUES(" . $host_id . ",'" . mysql_real_escape_string($name) . "')";
			echo $query . "<br />";
